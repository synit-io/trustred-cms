...
- Prefer `cross-env` for package scripts that set environment variables.
- Prefer `cross-env-shell` for package scripts that combine multiple commands or shell operators.
- Avoid adding raw `npm` or `pnpm` command chains to `package.json` scripts when the same script can be wrapped with `cross-env` or `cross-env-shell`.
...
## Resources
- Example PayloadCMS Backend/Frontend Project: `_examples_/payload_example`
