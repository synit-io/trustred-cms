...
type FeedSource = 'crew' | 'equipment' | 'events' | 'faqs' | 'operations' | 'posts'
...
function getPagePath(page: Pick<Page, 'breadcrumbs' | 'slug' | 'title'>) {
  const breadcrumbUrl = page.breadcrumbs?.[page.breadcrumbs.length - 1]?.url
  if (breadcrumbUrl) {
    return normalizePublicPath(breadcrumbUrl)
  }

  const publicSlug = resolvePublicSlug(page.slug, String(page.title ?? ''))
...
export const getPageBySlug = cache(async (slug: string) => {
  const normalizedPath = slug === 'home' ? '/' : normalizePublicPath(slug)
  const pathSegments = normalizedPath.split('/').filter(Boolean)
  const normalizedSlug = pathSegments[0] ?? 'home'
  const pageIdentity: Where =
...
export const getPublicOperationById = cache(async (id: string) => {
  ...
  return (result.docs[0] as Operation | undefined) ?? null
})
...
export const getPublicPostBySlug = cache(async (slug: string) => {
  ...
  return (
    (legacyResult.docs as Post[])
      .map((item) => withResolvedPostSlug(item))
      .find((item) => sameNormalizedSlug(item.slug, normalizedSlug)) ?? null
  )
})
...
export const getPublicEventBySlug = cache(async (slug: string): Promise<PayloadEvent | null> => {
  ...
  return (
    (legacyResult.docs as PayloadEvent[])
...
export const getPublicEquipmentBySlug = cache(async (slug: string) => {
  ...
  return (
    (legacyResult.docs as Equipment[])
...
export const getPublicEquipmentById = cache(async (id: number | string) => {
  ...
  return (result.docs[0] as Equipment | undefined) ?? null
})
...
export const getPublicCrewById = cache(async (id: string) => {
  ...
  return (result.docs[0] as Crew | undefined) ?? null
})
...
export const getPublicFaqById = cache(async (id: string) => {
  ...
  return (result.docs[0] as Faq | undefined) ?? null
})
