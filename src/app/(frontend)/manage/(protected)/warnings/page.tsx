...
import { getEditorialPermissions, requireEditorialContext, saveWarningSettings } from '@/lib/trustred/editorial'
    ...
    <div className="grid gap-6">
      <section className="ff-card">
        ...
        <p className="mt-4 max-w-3xl text-neutral-700">
          Presets liegen jetzt als eigene Payload-Collection vor. Das Admin-Backend kann alle Datensätze vollständig bearbeiten. Diese Frontend-Oberfläche zeigt System-Presets bewusst kompakt und schreibgeschützt, während nur Custom-Presets hier detailliert gepflegt werden.
        </p>
        <p className="mt-3 max-w-3xl text-sm text-neutral-600">
          DWD-Systemwerte basieren auf dem bisherigen Trustred-Presetkatalog und den offiziellen DWD-Einbindungsdaten aus{' '}
          <a className="text-[var(--brand-600)] underline" href={DWD_OBJECT_EMBED_DOCS_URL} rel="noreferrer" target="_blank">
            Ihr Homepagewetter
          </a>
          . NINA-Bundesland-Defaults folgen der offiziellen{' '}
          <a className="text-[var(--brand-600)] underline" href={NINA_API_DOCS_URL} rel="noreferrer" target="_blank">
            NINA API
          </a>
          .
        </p>
      ...
      <section className="ff-card grid gap-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {[dwdHealth, ninaHealth].filter(Boolean).map((snapshot) => (
            <article className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4" key={`${snapshot?.provider}-${snapshot?.regionLabel}`}>
              ...
              <p className="mt-3 text-base font-semibold text-neutral-900">
                Status: {snapshot?.status === 'live' ? 'Live' : snapshot?.status === 'fallback' ? 'Fallback' : 'Fehler'}
              </p>
              <p className="mt-2 text-sm text-neutral-700">
                {snapshot?.entries.length ?? 0} Warnungen · Aktualisiert {snapshot?.updatedAt ? new Date(snapshot.updatedAt).toLocaleString('de-DE') : 'unbekannt'}
              </p>
              ...
              <p className="mt-2 text-xs font-semibold uppercase tracking-[0.12em] text-neutral-500">
                Quelle: {snapshot?.sourceState === 'ok' ? 'Stabil' : snapshot?.sourceState === 'empty' ? 'Leer, aber erreichbar' : 'Degradiert'}
              </p>
              {snapshot?.note ? (
      ...
      <section className="ff-card grid gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-600">
            Die Liste ist standardmäßig eingeklappt. Bei Bedarf kann jedes Preset aufgeklappt werden, um die hinterlegten Werte zu prüfen.
          </p>
        </div>
        <div className="grid gap-3">
          {systemPresets.map((preset) => (
            <details className="rounded-[1.2rem] border border-neutral-200 bg-white p-4" key={preset.key}>
              ...
              <div className="mt-4 grid gap-3 text-sm text-neutral-700 md:grid-cols-2">
                <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3">
                  <p className="text-xs font-bold uppercase tracking-[0.12em] text-neutral-500">Schlüssel</p>
                  <p className="mt-2 break-all">{preset.key}</p>
                </div>
                <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3">
                  <p className="text-xs font-bold uppercase tracking-[0.12em] text-neutral-500">Quelle</p>
                  <p className="mt-2 break-all">{preset.sourceUrl || 'Nicht gesetzt'}</p>
                </div>
                {preset.provider === 'dwd' ? (
                  <>
                    <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3">
                      <p className="text-xs font-bold uppercase tracking-[0.12em] text-neutral-500">Bundesländer</p>
                      <p className="mt-2">{(preset.dwdStates ?? []).map((entry) => entry.state).join(', ') || 'Nicht gesetzt'}</p>
                    </div>
                    <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3">
                      <p className="text-xs font-bold uppercase tracking-[0.12em] text-neutral-500">Karten</p>
                      <p className="mt-2">
                        Warnkarte: {preset.warningMapUrl ? 'Ja' : 'Nein'} · Wetterkarte: {preset.weatherMapUrl ? 'Ja' : 'Nein'} · Waldbrandkarte: {preset.wildfireMapUrl ? 'Ja' : 'Nein'}
                      </p>
                    </div>
                  </>
                ) : (
                  <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3">
                    <p className="text-xs font-bold uppercase tracking-[0.12em] text-neutral-500">NINA ARS</p>
      ...
      <form action={action} className="ff-card grid gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-600">
            Hier werden ausschließlich eigene Presets bearbeitet. Diese Datensätze sind anschließend ebenfalls im Payload-Admin als Collection-Einträge sichtbar.
          </p>
        </div>
        <input name="warningPresets.count" type="hidden" value={warningPresetRows.length} />
        <div className="grid gap-4">
          {warningPresetRows.map((preset, index) => (
            <details className="rounded-[1.2rem] border border-neutral-200 bg-neutral-50 p-4" key={`warning-preset-${index}`} open={Boolean(preset.label || preset.regionLabel)}>
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4">
                <div>
                  <h4 className="text-lg">{preset.label || `Custom Preset ${index + 1}`}</h4>
                  <p className="text-sm text-neutral-600">{preset.regionLabel || 'Region offen'} · {String(preset.provider ?? 'dwd').toUpperCase()}</p>
                </div>
              ...
              <div className="mt-4 ff-form-grid">
                <label>
                  Schlüssel
                  <input className="ff-input" defaultValue={String(preset.key ?? '')} name={`warningPresets.${index}.key`} />
                </label>
                <label>
                  Anzeige-Label
                  <input className="ff-input" defaultValue={String(preset.label ?? '')} name={`warningPresets.${index}.label`} />
                </label>
                <label>
                  Anbieter
                  <select className="ff-input" defaultValue={String(preset.provider ?? 'dwd')} name={`warningPresets.${index}.provider`}>
                ...
                <label>
                  Regionslabel
                  <input className="ff-input" defaultValue={String(preset.regionLabel ?? '')} name={`warningPresets.${index}.regionLabel`} />
                </label>
                <label>
                ...
                <label>
                  ...
                  <textarea
                    className="ff-input"
                    defaultValue={(preset.dwdRegionIds ?? []).map((region) => region.regionId).join('\n')}
                ...
                <label>
                  DWD Forecast-URL
                  <input className="ff-input" defaultValue={String(preset.forecastUrl ?? '')} name={`warningPresets.${index}.forecastUrl`} />
                </label>
                <label>
                  DWD Warnkarten-URL
                  <input className="ff-input" defaultValue={String(preset.warningMapUrl ?? '')} name={`warningPresets.${index}.warningMapUrl`} />
                </label>
                <label>
                  DWD Wetterkarten-URL
                  <input className="ff-input" defaultValue={String(preset.weatherMapUrl ?? '')} name={`warningPresets.${index}.weatherMapUrl`} />
                </label>
                <label>
                  DWD Waldbrandkarten-URL
                  <input className="ff-input" defaultValue={String(preset.wildfireMapUrl ?? '')} name={`warningPresets.${index}.wildfireMapUrl`} />
                </label>
                <label>
                  NINA ARS
                  <input className="ff-input" defaultValue={String(preset.ninaArs ?? '')} name={`warningPresets.${index}.ninaArs`} />
                </label>
                <label>
                  Quell-URL
                  <input className="ff-input" defaultValue={String(preset.sourceUrl ?? '')} name={`warningPresets.${index}.sourceUrl`} />
                </label>
              </div>
            </details>
          ))}
